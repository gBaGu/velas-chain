...
#[derive(Clone, Debug)]
pub struct EvmContractToPayerKeys {
    pub filter: evm_gas_station::instruction::TxFilter,
    pub gas_station_payer: Pubkey,
    pub storage_acc: Pubkey,
}
...
pub struct EvmBridge {
    ...
    redirect_to_proxy_filters: HashMap<Address, Vec<EvmContractToPayerKeys>>,
}
...
impl EvmBridge {
    pub fn new(
        ...
        Self {
            ...
            redirect_to_proxy_filters: Default::default(),
        }
    }
    ...
    pub fn get_gas_station_program_id(&self) -> Option<Pubkey> {
        self.gas_station_program_id
    }
    ...
    pub fn set_redirect_to_proxy_filters(
        &mut self,
    ...
    pub async fn fetch_gas_station_filters(
        &self,
        gas_station_id: &Pubkey,
    ) -> HashMap<Address, Vec<EvmContractToPayerKeys>> {
        let mut filter_map: HashMap<_, Vec<_>> = HashMap::default();
        for (pubkey, acc) in self
            .rpc_client
            .get_program_accounts(gas_station_id)
            .await
            .unwrap()
        {
            let acc: evm_gas_station::state::Payer =
                match BorshDeserialize::deserialize(&mut acc.data.borrow()) {
                    Ok(acc) => acc,
                    Err(_) => continue,
                };
            for filter in acc.filters {
                filter_map
                    .entry(filter.get_contract())
                    .or_default()
                    .push(EvmContractToPayerKeys {
                        filter,
                        gas_station_payer: acc.payer,
                        storage_acc: pubkey,
                    });
            }
        }
    ...
    pub async fn find_gas_station_filter(
        &self,
        tx: &evm_gas_station::evm_types::Transaction,
    ) -> Option<EvmContractToPayerKeys> {
        if let evm_gas_station::evm_types::TransactionAction::Call(contract) = tx.action {
            if let Some(filters) = self.redirect_to_proxy_filters.get(&contract) {
                let filters: Vec<_> = filters
                    .iter()
                    .filter(|val| val.filter.is_match(tx))
                    .collect();
                if filters.is_empty() {
                    return None;
                }
                ...
                for filter in filters {
                    let bal = match self
                        .get_rpc_client()
                        .get_balance(&filter.gas_station_payer)
                        .await
                    {
                        Ok(balance) => balance,
                        Err(err) => {
                            error!("Error occurred while fetching payers balances: {}", err);
                            return None;
                        }
                    };
                    if bal > max_balance {
