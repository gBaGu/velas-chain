...
async fn process_tx(
    ...
    let gas_station_tx = evm_gas_station::evm_types::Transaction {
        nonce: tx.nonce,
    ...
    let instructions = if let Some(filter) = bridge.find_gas_station_filter(&gas_station_tx).await {
        info!(
            "Bridge filters matched. Sending transaction {} to gas station {}",
            tx.tx_id_hash(),
            bridge.get_gas_station_program_id().unwrap()
        );
        vec![evm_gas_station::execute_tx_with_payer(
            gas_station_tx,
            bridge.get_gas_station_program_id().unwrap(),
            bridge.get_key().pubkey(),
            filter.storage_acc,
            filter.gas_station_payer,
        )]
    } else {
